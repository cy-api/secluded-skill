...
本技能包沉淀了 Secluded 词库开发的完整规范：语法结构、变量字节规则、转义、流程控制、函数清单、菜单与权限设计、接口 JSON 取值，以及词库校验规范（AI 可直接执行，无需 Python 环境）。目标是让任何 AI 拿到本包后，都能写出**符合规范、可直接加载运行、无多余污染**的词库文件。
...
```
secluded-skill/
├── SKILL.md                          # 主技能规范（frontmatter + 13 章 + 子技能路由）
├── AGENTS.md                         # 技能使用说明（Codex/Claude 兼容入口）
├── SOUL.md                           # 技能人格定义
├── README.md                         # 项目说明（本文件）
├── CHANGELOG.md                      # 版本迭代记录
├── .gitignore                        # git 忽略规则
...
│   ├── 发言统计.txt                   # 参考词库：统计排行
│   ├── 钓鱼娱乐.txt                   # 参考词库：娱乐玩法
│   └── 词库校验规范.md               # 词库校验规则（AI 直接执行，无需 Python 环境）
└── skills/                           # 子技能目录
    ├── secluded-skill-admin/skill.md # 子技能：管理功能域
    └── secluded-skill-fun/skill.md   # 子技能：娱乐功能域
```
...
3. **按功能域加载子技能**：群管/统计类加载 `skills/secluded-skill-admin/skill.md`，娱乐类加载 `skills/secluded-skill-fun/skill.md`；再对照 `references/` 下参考词库模仿结构
4. **编写词库**：遵守标准作业流程（确认需求 → 查规范 → 设计结构 → 编写 → 自检 → 交付）
5. **自检**：按 `references/词库校验规范.md` 逐项检查词库，清零错误后交付（无需 Python 环境）
...
按 `references/词库校验规范.md` 逐项自检（AI 直接执行，无需 Python 环境），检查项包括：
...
- **菜单项名 = 触发指令**：菜单显示的功能名必须与词库词汇头一字不差，看到什么就发什么
- 换行必须用 `\n`（文件内直接换行会被引擎拼接）
...
A skill pack for the **Secluded framework** (QQ bot word libraries / custom replies). It provides complete syntax rules, variable byte rules, escaping, control flow, function reference, menu & permission design guidelines, API JSON extraction conventions, and an AI-executable checking spec (no Python environment needed) to guarantee runnable `.txt` word libraries.
...
- ✅ Menu design standards: menu item = trigger, three-level structure, four-character names, two per line, `\n` line breaks
...
- ✅ AI-executable checking spec (no Python environment needed)
...
```
secluded-skill/
├── SKILL.md                          # Main spec (frontmatter + 13 chapters + sub-skill routing)
├── AGENTS.md                         # Usage guide (Codex/Claude compatible entry)
...
│   ├── 群管词库.txt                   # Reference: group admin
│   ├── 发言统计.txt                   # Reference: statistics & ranking
│   ├── 钓鱼娱乐.txt                   # Reference: entertainment games
│   └── 词库校验规范.md               # Checking spec (AI-executable, no Python needed)
└── skills/                           # Sub-skills
    ├── secluded-skill-admin/skill.md # Sub-skill: admin domain
    └── secluded-skill-fun/skill.md   # Sub-skill: fun domain
```
...
5. Self-check against `references/词库校验规范.md`; clear all errors before delivery (no Python needed)
...
Check word libraries against `references/词库校验规范.md` — an AI-executable rule list that needs no Python environment. It covers: variable byte violations (error), unbalanced `%` (error), AI-generated disclaimers at the end (error), over-broad headers (warning), undefined variables (warning), consecutive blank lines (warning), suspected function nesting (warning), trailing `\n` (warning).
