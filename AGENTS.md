...
2. 按 `SKILL.md` 第 12 章标准作业流程执行：确认需求 → 查变量大全/参考词库 → 设计结构 → 编写 → 运行校验脚本自检 → 交付
...
2. Follow the standard workflow in `SKILL.md` §12: confirm requirements → consult the variable encyclopedia/reference libraries → design structure → write → run the check script → deliver.
