---
name: secluded-skill
description: 按 Secluded 框架规范编写可直接加载运行的 QQ 机器人词库。当用户要求编写、修改、校验 Secluded 词库 / 自定义回复 / 机器人词汇块，或提到词库语法、变量、转义、菜单、权限、接口对接、子技能时使用。包含完整语法规范、变量字节规则、转义、流程控制、函数清单、菜单与权限设计、接口 JSON 取值、子技能路由与词库校验规范（无需 Python 环境）。
version: 1.0.3
author: chiyan
---
...
引擎按**字节**判定变量名合法性：**键名只认 1 字节或 3 字节**。

| 键名 | 字节数 | 是否合法 | 示例 |
|---|---|---|---|
| `A`、`B`、`I`、`s` | 1 | ✅ | `A:666` |
| `API`、`TXT`、`PKG`、`CUR` | 3 | ✅ | `API:$访问 url$` |
| `AAA`、`123`、`Ab0`（3 位 ASCII） | 3 | ✅ | `123:值` |
...
1. **形式1：单字节做键、多字节做值**：`K:V`，键一般为字母或数字，如 `A:666`
2. **形式2：三字节做键、多字节做值**：键为 3 个 ASCII 字符或 1 个汉字，如 `TXT:内容`、`名:标题`、`123:值`；注意 `键:值` 写法中冒号左边是 1 或 3 字节才算赋值
3. **形式3：多字节做键、多字节做值**：`$变量 键 值$`，弥补前两种不足，如 `$变量 哈哈哈666 我是内容$`

### 2.3 取值

- 普通取值：`%变量名%`，如 `%A%`、`%TXT%`
- JSON 取值：`@变量名[键/索引]`，**变量名必须是 1 或 3 字节**，多层路径用 `[键]` 逐级连接，如 `b:@A[bf][c]`、`d:@b[1]`、`e:@A[bf][c][0]`
- 复杂 JSON 也可用 `$JSON 解析 变量名 路径...$`（变量名任意，见变量大全）

### 2.4 内置变量（无需自己定义，动手前先 grep 变量大全确认）

| 类别 | 内置变量 |
|---|---|
| 账号 | `%登录账号%`、`%Account%`、`%Robot%`（机器人/登录账号） |
| 操作者 | `%QQ%`、`%Uin%`（发送者 QQ）、`%昵称%`、`%UinName%`（发送者昵称） |
| 群 | `%群号%`、`%GroupId%`、`%群%`（群号） |
| 消息 | `%MSG%`（原始消息）、`%MSGJ%`（消息 JSON） |
| 参数 | `%参数1%`、`%参数2%`…（空格分隔参数）、`%参数量%`（参数个数）、`%参数-1%`（原始内容） |
| 正则捕获 | `%括号1%`、`%括号2%`…（括号捕获）、`%括号量%`（捕获个数） |
| 其他 | `%群主QQ%`、`%时间戳毫秒%`、`%时间戳进程%`、`%上线模式%`、`%消息来源%`、`%随机数%`（随机 int，范围见大全） |

### 2.5 转义

- 文本中非变量引用的 `%` 必须写成 `\%`：错误 `$写 应用 版本 v1%201.0.0$`（`%20` 被当变量解析），正确 `$写 应用 版本 v1\%201.0.0$`
- 整段转义（变量大全原例）：`\%昵称\%(%QQ%) 好的`——想输出字面 `%昵称%` 而非取值时，前后都转义

### 2.6 行变量

`#->var:` 结构可把多行内容组成一个变量（变量大全"行变量"块）：
...
非法变量名：赋值行不会被识别为赋值，而是**原样输出给用户**（真实踩坑案例："当前值:5" 被发到群里）
...
- **菜单项名 = 触发指令（硬规则）**：菜单里展示的每个功能名，必须与词库中对应词汇头**一字不差**；用户看到什么就发什么。禁止"菜单写 A、实际触发 B"；子菜单同理
...
正确示例（词库文件中的写法，实际输出即换行效果；每个菜单项必须存在同名词汇块）：
...
词库中必须同时存在 `许愿功能`、`娱乐功能`、`群管功能`、`视频功能` 四个**同名词汇块**。子菜单同理——`许愿功能` 块内输出"许愿 捞愿望"，词库就必须有 `许愿(.*)` 词汇头：
...
7. **菜单项≠触发指令**：菜单显示的功能名必须与词库词汇头一致（如菜单写"禁言"，词库必须有 `禁言(.*)` 块），否则用户发菜单项无法触发
8. **路径分隔符**：自定义路径用 `/` 或 `\\` 通用
9. **输出即回复**：词汇体里的普通文本行就是回复内容；不想回复用 `返回` 结束
...
6. **自检**：写完按 `references/词库校验规范.md` 逐项检查，清零全部错误后交付
...
#### 2.1 Variable Name Rules (byte-based)

Engine validates variable names by **byte length**: a **key must be 1 byte or 3 bytes**.

| Key | Bytes | Valid | Example |
|---|---|---|---|
| `A`, `B`, `I`, `s` | 1 | ✅ | `A:666` |
| `API`, `TXT`, `PKG`, `CUR` | 3 | ✅ | `API:$访问 url$` |
| `AAA`, `123`, `Ab0` (3 ASCII chars) | 3 | ✅ | `123:值` |
| `名` (one CJK char) | 3 | ✅ | `名:@API[name]` |
| `当前值` (3 CJK chars) | 9 | ❌ | treated as text output |
| `AA` (2), `ABCD` (4) | 2/4 | ❌ | invalid |

#### 2.2 Three Assignment Forms (as defined in the encyclopedia)

1. **Form 1: 1-byte key, multi-byte value**: `K:V`, key usually letter/digit, e.g. `A:666`
2. **Form 2: 3-byte key, multi-byte value**: key is 3 ASCII chars or 1 CJK char, e.g. `TXT:内容`, `名:标题`, `123:值`; in `键:值` lines, the left side must be 1 or 3 bytes to count as an assignment
3. **Form 3: multi-byte key & value**: `$变量 键 值$`, fills the gap, e.g. `$变量 哈哈哈666 我是内容$`

#### 2.3 Reading Values

- Normal: `%变量名%`, e.g. `%A%`, `%TXT%`
- JSON: `@变量名[键/索引]` — **variable name must be 1 or 3 bytes**; chain nested paths with `[键]`, e.g. `b:@A[bf][c]`, `d:@b[1]`, `e:@A[bf][c][0]`
- Complex JSON can also use `$JSON 解析 变量名 路径...$` (any variable name; see the encyclopedia)

#### 2.4 Built-in Variables (no need to define; grep the encyclopedia before coding)

| Category | Built-ins |
|---|---|
| Account | `%登录账号%`, `%Account%`, `%Robot%` (bot/login account) |
| Operator | `%QQ%`, `%Uin%` (sender QQ), `%昵称%`, `%UinName%` (sender nickname) |
| Group | `%群号%`, `%GroupId%`, `%群%` (group number) |
| Message | `%MSG%` (raw message), `%MSGJ%` (message JSON) |
| Params | `%参数1%`, `%参数2%`… (space-split params), `%参数量%` (count), `%参数-1%` (raw content) |
| Regex captures | `%括号1%`, `%括号2%`…, `%括号量%` (capture count) |
| Others | `%群主QQ%`, `%时间戳毫秒%`, `%时间戳进程%`, `%上线模式%`, `%消息来源%`, `%随机数%` (random int, range in encyclopedia) |

#### 2.5 Escaping

- Any `%` in text that is NOT a variable reference must be written `\%`: wrong `$写 应用 版本 v1%201.0.0$` (`%20` parsed as a variable), right `$写 应用 版本 v1\%201.0.0$`
- Whole-token escaping (encyclopedia example): `\%昵称\%(%QQ%) 好的` — escape both sides to output a literal `%昵称%` instead of reading it

#### 2.6 Row Variables

The `#->var:` structure packs multiple lines into one variable (encyclopedia "行变量" block):
...
#### 2.8 Consequence of Invalid Names

The line is NOT treated as assignment and is output verbatim (real case: "当前值:5" was sent to the group)
...
- **Menu item = trigger (hard rule)**: every item shown in a menu must match the word block header **exactly**. Users send what they see — never show A but trigger B. Same for sub menus.
...
Good (every menu item must have an identically named word block):
...
The library must contain identically named blocks `许愿功能`, `娱乐功能`, `群管功能`, `视频功能`. Sub menus follow the same rule — if the `许愿功能` block outputs "许愿 捞愿望", the library must have a `许愿(.*)` header.

Bad: numbered lists, "send: XXX" hints, or menu items that don't match the actual triggers — all violate the rule.
...
Menu items in admin menus must match real headers too — e.g. if the menu shows "禁言", the library must have a `禁言(.*)` block.
...
9. Menu item ≠ trigger: menu text must match the word header exactly (e.g. show "禁言" → library must have `禁言(.*)`), otherwise the item won't trigger
...
5. Self-check: verify item by item against `references/词库校验规范.md`; clear all errors before delivery
